...
export default function GroupScheduleViewNew({ group }: GroupScheduleViewProps) {
  ...
  // スロットの内容を取得
  const getSlotContent = (day: string, period: string) => {
    const slot = getSlotInfo(day, period);
    if (!slot) return null;
    ...
    if (slot.isFree) {
      return (
        <div className="text-center">
          ...
          <p className="text-xs text-green-400 font-medium">全員OK</p>
        </div>
      );
    } else {
      return (
        <div className="text-center">
          <p className="text-lg font-semibold text-white">{free}/{total}</p>
          <p className="text-xs text-white/60">空き</p>
        </div>
      );
    }
  };
                ...
                <table className="w-full">
                  <thead>
                    <tr className="bg-white/5">
                      <th className="p-3 text-center text-white/80 font-medium border-b border-white/10">
                        時限
                      </th>
                      {days.map((day) => (
                        <th key={day} className="p-3 text-center text-white/80 font-medium border-b border-white/10 min-w-[100px]">
                          {day}曜日
                        </th>
                      ))}
                    </tr>
                  ...
                  <tbody>
                    {periods.map((period) => (
                      <tr key={period} className="border-b border-white/5">
                        <td className="p-3 text-center bg-white/5">
                          <div className="text-white font-medium">{period}限</div>
                          <div className="text-xs text-white/50">{periodTimes[period as keyof typeof periodTimes]}</div>
                        </td>
                        {days.map((day) => (
              ...
              <CardContent>
                <div className="space-y-2">
                  <div className="flex items-center gap-3">
                    <div className="w-4 h-4 bg-green-500/20 border-2 border-green-400/30 rounded"></div>
                    <span className="text-sm text-white/70">全員が空いている</span>
                  </div>
                  <div className="flex items-center gap-3">
                    <div className="w-4 h-4 bg-orange-500/20 border-2 border-orange-400/30 rounded"></div>
                    <span className="text-sm text-white/70">一部のメンバーが空いている</span>
                  </div>
                  <div className="flex items-center gap-3">
                    <div className="w-4 h-4 bg-yellow-500/20 border-2 border-yellow-400/30 rounded"></div>
                    <span className="text-sm text-white/70">半数以上が空いている</span>
                  </div>
                  <div className="flex items-center gap-3">
                    <div className="w-4 h-4 bg-red-500/20 border-2 border-red-400/30 rounded"></div>
                    <span className="text-sm text-white/70">ほとんど空いていない</span>
                  </div>
                </div>
              </CardContent>
            </Card>
